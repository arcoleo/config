...
add-zsh-hook precmd _work_kube_ps1_update_cache
zmodload zsh/stat
...
_work_kube_ps1_binary_check() {
  command -v "$1" >/dev/null
}

_work_kube_ps1_symbol() {
...
_work_kube_ps1_split() {
  type setopt >/dev/null 2>&1 && setopt SH_WORD_SPLIT
  local IFS=$1
  echo $2
}

_work_kube_ps1_file_newer_than() {
  local mtime
  local file=$1
  local check_time=$2
...
_work_kube_ps1_update_cache() {
  KUBECONFIG="${KUBECONFIG:=$HOME/.kube/config}"
  if ! _work_kube_ps1_binary_check "${KUBE_PS1_BINARY}"; then
  ...
  local conf
  for conf in $(_work_kube_ps1_split : "${KUBECONFIG:-${HOME}/.kube/config}"); do
    [[ -r "${conf}" ]] || continue
    if _work_kube_ps1_file_newer_than "${conf}" "${KUBE_PS1_LAST_TIME}"; then
      _work_kube_ps1_get_context_ns
      return
    fi
  done
...
_work_kube_ps1_get_context_ns() {
  ...
  KUBE_PS1_CONTEXT="$(${KUBE_PS1_BINARY} config current-context | cut -d '.' -f 1 2>/dev/null)"
...
work_kube_ps1 () {
  ...
  KUBE_PS1+="${KUBE_PS1_COLOR_SYMBOL}$(_work_kube_ps1_symbol)"
